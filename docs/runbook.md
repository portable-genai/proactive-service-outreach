...
`OUTREACH_S2S_TOKEN`. With routing on and the URL unset, the managed profile REFUSES TO BOOT, so a
...
`OUTREACH_REVIEW_ROUTING` switches routing, read in three states: unset is on, `true`/`false` (or
`on`/`off`) wins, and an emptied or unrecognised value refuses at boot. Off needs no console, logs
one warning at startup, and every outreach held for review reports `review_routing: "off"` with an
empty `review_ref`. A hand-off that fails at request time does not fail the request: the response
