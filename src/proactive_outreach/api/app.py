"""FastAPI application for Proactive Service Outreach (E5).
...
``OUTREACH_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``OUTREACH_S2S_TOKEN`` is wrong in the most dangerous direction:
...
header and no ``X-Dev-Persona`` then got the seeded persona list and a real outreach decision.
...
_CORS_ORIGINS_ENV = "OUTREACH_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "OUTREACH_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "OUTREACH_S2S_AUDIENCE"
_API_HOST_ENV = "OUTREACH_API_HOST"
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``OUTREACH_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``OUTREACH_PROFILE`` must never be allowed to
    ...
    ``OUTREACH_ALLOW_INSECURE_DEMO=1``.
    """
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "OUTREACH_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_SERVICE_NAME = "proactive-service-outreach"
...
_APP_TITLE = "Proactive Service Outreach"
...
# Which model answered, and whether it searched: the drafting adapters note it as they call
# (`hex_service_kit.provenance.note_model`; the offline template drafter notes its stub name) and
# this emits it as `X-Answered-By` / `X-Search-Used` on the same response. The console's pills
# read those two headers, so what a pill names is what answered, never what configuration says
# would. A request that drafted nothing (an ineligible contact is never drafted) sends neither,
...
#
# Note what is NOT in this expression: OUTREACH_S2S_TOKEN. A service
...
def _service(container: Container) -> OutreachService:
    """The one assembly of the pipeline. Every surface goes through it, in this order."""
...
def _tenant_for(principal: Principal, container: Container) -> str:
    """The tenant an event is evaluated under: the VERIFIED principal's, never the body's."""
    return principal.tenant or container.settings.tenant


# NO SURFACE ACCEPTS AN ``as_of`` FROM ITS CALLER, and none of the request schemas carries one.
#
# The domain always takes an explicit instant, which is what makes a decision replayable, so it
# is tempting to let a caller pin it and get replay for free on the API too. That instant is
# what QUIET HOURS are evaluated against. A caller who could choose it could ask for a decision
# "as at 14:00" at 23:00 and have the engine agree that the market was open, which turns the one
# rule that exists to protect a customer from being telephoned in the night into a parameter.
#
# It matters most on the agent surface, where the caller is a model and the whole architecture
# exists so that a prompt-injected model cannot reach a consequential lever. So the rule is the
# same everywhere and has no exception: every SURFACE decides against the wall clock. Pinning an
# instant stays available to the eval, the tests and the demo, which call the domain directly
# and contact nobody.


@app.post("/v1/outreach/evaluate", response_model=OutreachResponse, tags=["artifacts"])
def evaluate(
    request: EventRequest,
    principal: Annotated[Principal, Depends(get_principal)],
) -> OutreachResponse:
    """Decide one operational event: trigger, consent, eligibility, draft, deliver or hold.

    The tenant comes from the VERIFIED principal and never from the request body, so a caller
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"{request.event_type!r} is not an event type this service watches for",
        ) from exc
    event = ServiceEvent(
        event_id=request.event_id,
        event_type=event_type,
        tenant=_tenant_for(principal, container),
...
def sweep(
    ...
    """Evaluate every detected event for the caller's tenant, routing each escalation."""
    container = _container()
    query = EventQuery(
        tenant=_tenant_for(principal, container),
        since=request.since,
    ...
    try:
        results = _service(container).sweep(query, actor=principal.actor)
    except EventSourceUnavailableError as exc:
        raise HTTPException(
...
@app.post("/v1/outreach/events", response_model=list[EventModel], tags=["artifacts"])
def events(
    request: SweepRequest,
    principal: Annotated[Principal, Depends(get_principal)],
) -> list[EventModel]:
    """List the events the detection port sees, WITHOUT the free-text detail.

    The detail field is the only place personal data reaches this service, so a listing that
    returned it would put an identifier on a screen for a reader who only wanted to know what
    happened. Evaluation redacts it; listing simply does not carry it.
    """
    container = _container()
    query = EventQuery(
        tenant=_tenant_for(principal, container),
    ...
    try:
        detected = container.events.detect(query)
    except EventSourceUnavailableError as exc:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail=str(exc)
        ) from exc
    return [EventModel.from_domain(event) for event in detected]
