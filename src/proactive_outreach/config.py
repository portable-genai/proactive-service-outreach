...
One env var (``OUTREACH_PROFILE``) selects the adapter family for every
...
from .domain.policy import DEFAULT_POLICY, OutreachPolicy
from .envread import boolean_setting, setting_or_default
from .ports.audit import AuditSinkPort
from .ports.consent import ConsentPort
from .ports.delivery import MessageDeliveryPort
from .ports.drafting import DraftingPort
...
from .ports.speech import TextToSpeechPort

_PROFILE_ENV = "OUTREACH_PROFILE"
_SETTINGS_ENV = "OUTREACH_SETTINGS"
...
_GENERATOR_PORT: str = "drafting"

#: What ``generator_model`` reports when the generator port is bound to the offline template
#: drafter, and what that drafter notes as the model that answered (``X-Answered-By``). Spelled
#: ONCE here so the pill's configured value and its answered value cannot drift apart.
...
#: chosen per repository. Resolving it from a path named ONCE here keeps the model pill reading the
#: same value the adapter passes to the model call, instead of a second copy that drifts.
_GENERATOR_MODEL_ATTR: str = "drafting_model"
...
def _model_from_settings(settings: object, path: str) -> str:
    ...
    (``settings.drafting_model`` here). There is deliberately no second, "harder" model a flag
    could swap in: a resolver that named a model the adapter never read would put a model on
    screen that never answered.
    """
...
class ProfileChoice:
    ...
    The variable is ``OUTREACH_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("OUTREACH_PROFILE", "local")``: that fallback reads an
...
DEFAULT_BINDINGS: dict[str, dict[str, str]] = {
    ...
    "consent": {
        "local": f"{_PKG}.adapters.local.consent:LocalConsentAdapter",
        "gcp": f"{_PKG}.adapters.gcp.consent:RemoteConsentAdapter",
        "onprem": f"{_PKG}.adapters.onprem.consent:OnPremConsentAdapter",
    },
    "delivery": {
        "local": f"{_PKG}.adapters.local.delivery:LocalChatDelivery",
        "gcp": f"{_PKG}.adapters.gcp.delivery:ConversationChannelDelivery",
        "onprem": f"{_PKG}.adapters.onprem.delivery:OnPremDelivery",
    },
    "drafting": {
        "local": f"{_PKG}.adapters.local.drafting:TemplateDraftingAdapter",
        "gcp": f"{_PKG}.adapters.gcp.drafting:VertexDraftingAdapter",
        "onprem": f"{_PKG}.adapters.onprem.drafting:OnPremDraftingAdapter",
    },
    "events": {
        "local": f"{_PKG}.adapters.local.events:FixtureEventSource",
        "gcp": f"{_PKG}.adapters.gcp.events:WarehouseEventSource",
        "onprem": f"{_PKG}.adapters.onprem.events:OnPremEventSource",
    },
    "identity": {
    ...
    "speech": {
        "local": f"{_PKG}.adapters.local.speech:FixtureSpeechSynthesis",
        "gcp": f"{_PKG}.adapters.gcp.speech:CloudTextToSpeech",
        "onprem": f"{_PKG}.adapters.onprem.speech:OnPremSpeechSynthesis",
    },
}
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``OUTREACH_SETTINGS``) that does not
...
REVIEW_ROUTING_ENV = "OUTREACH_REVIEW_ROUTING"
...
class Settings:
    ...
    consent_url: str = ""
    #: A JSON file of synthetic events for the offline detection adapter. Empty means the
    #: shipped fixture set; a path that is NAMED but missing is an error, not a fall-back.
    event_fixture_path: str = ""
    #: The client-owned warehouse view the managed detection adapter reads events from.
    event_view: str = ""
    #: The managed model that drafts notification bodies. Empty means no drafter is reachable,
    #: which the service treats as a discarded draft: deterministic body, human review, no send.
    drafting_model: str = ""
    #: The managed conversation channel a chat notification is delivered on.
    chat_agent: str = ""
    #: The managed voice and the in-region location synthesised audio is written to. Audio never
    #: enters this process's domain layer; only a reference to it does.
    speech_voice: str = ""
    speech_output_uri: str = ""
    #: The BANK's policy, not this codebase's: trigger rules, frequency caps, quiet hours,
    #: message templates, the suppression list and the body length cap. Loaded from the
    #: ``policy:`` block of the settings file; see ``domain/policy.py`` for what each field
    #: does and which way it fails when nobody configured it.
    policy: OutreachPolicy = field(default_factory=lambda: DEFAULT_POLICY)
    ...
    def generator_model(self) -> str:
        ...
        (``X-Answered-By``, noted by the drafting adapter itself). So this must be the model the
        adapter calls: under ``gcp`` the setting its call reads, never a model a flag could swap
        in.
    ...
    def load(cls, path: Path | None = None) -> Settings:
        ...
        settings = cls(
            ...
            event_fixture_path=str(data.get("event_fixture_path") or ""),
            event_view=str(data.get("event_view") or ""),
            drafting_model=str(data.get("drafting_model") or ""),
            chat_agent=str(data.get("chat_agent") or ""),
            speech_voice=str(data.get("speech_voice") or ""),
            speech_output_uri=str(data.get("speech_output_uri") or ""),
            policy=OutreachPolicy.from_mapping(data.get("policy")),
...
class Container:
    ...
    @cached_property
    def consent(self) -> ConsentPort:
        adapter = self._bind("consent")
        assert isinstance(adapter, ConsentPort)
        return adapter

    @cached_property
    def delivery(self) -> MessageDeliveryPort:
        adapter = self._bind("delivery")
        assert isinstance(adapter, MessageDeliveryPort)
        return adapter

    @cached_property
    def drafting(self) -> DraftingPort:
        adapter = self._bind("drafting")
        assert isinstance(adapter, DraftingPort)
        return adapter

    @cached_property
    def events(self) -> EventDetectionPort:
        adapter = self._bind("events")
        assert isinstance(adapter, EventDetectionPort)
        return adapter
    ...
    @cached_property
    def speech(self) -> TextToSpeechPort:
        adapter = self._bind("speech")
