...
Proactive Service Outreach (E5): Consent-gated, frequency-capped service outreach from deterministic operational triggers.
Rendered from `hex-service-template`, so it starts at reference parity rather than converging
toward it. Package `proactive_outreach`, environment prefix
`OUTREACH`, region `asia-southeast1`.
...
- `src/proactive_outreach/domain/` is PURE stdlib. No web framework, no cloud SDK,
  no HTTP. `kernel.py` holds the vertical-neutral machinery; `models.py`, `policy.py`,
  `trigger_engine.py`, `eligibility.py`, `drafting.py` and `outreach_service.py` hold this
  vertical. A fork building a different vertical rewrites those and leaves `kernel.py`.
...
- **The exposure guard is derived from the IDENTITY BINDING, and from nothing else.** An end-user
  ...
  `OUTREACH_S2S_TOKEN` may never enter that decision: it authenticates a
  ...
  persona and a real outreach decision. `tests/unit/test_end_user_auth_posture.py` walks the
...
- **The one adapter that declares `VERIFIED` must EARN it, and it is the one adapter that may not
  ...
  configured `OUTREACH_IAP_AUDIENCE`, three-state: unset or emptied REFUSES,
...
- **One profile read, three states, refused at import.** `config.PROFILE_CHOICE` resolves once at
  ...
  `OUTREACH_PROFILE` (mentioning it in a refusal message is wanted, reading
...
- **Rule R8: escalations are ROUTED, never merely flagged.** Setting `requires_human_review` and
  ...
  and green.
- **The consequential decision is deterministic.** Whether a trigger fires, whether contact is
  permitted, the cap arithmetic, the severity band and whether a human must approve all come
  from pure stdlib code with an explicit `as_of` and no clock, and they replay exactly. An LLM
  phrases a message that has already been decided; it produces no number and no verdict.
- **Eligibility fails closed on ANY unknown consent state.** No decision at all, an outcome that
  is not exactly the allow token, a reason token this deployment does not recognise, a decision
  about a different question, a decision pinned to another instant, an unconfigured cap and an
  unconfigured market all DENY. `domain/eligibility.py` contains no branch that removes a reason
  and none that returns permission from an absent value; `tests/unit/test_eligibility.py` has a
  named test per shape.
- **The model may phrase and may not inform.** `ports/drafting.py` is the only seam a model is
  reachable through, it returns a `str`, and `OutreachService` calls it only inside the
  `eligible` branch. A draft is DISCARDED on any validation failure, never repaired and never
  sent with a warning; the deterministic body then goes to a human.
- **No surface accepts an `as_of` from its caller.** The domain takes an explicit instant so a
...
- **A held result is a result that was NOT delivered.** In this vertical, "requires_human_review"
  has to mean nothing went out, so the API, the CLI, the agent tools and the contract suite all
  assert the delivery did not happen, not merely that the flag was set.
