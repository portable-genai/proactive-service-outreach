...
def serving_body() -> dict[str, Any]:
    """The canonical request body for ``POST /v1/outreach/evaluate``.

    ONE home, because four suites post it and a body that drifted between them would have them
    asserting about different requests while claiming to assert about one. It carries the
    consequential event, timestamped now, so the two properties every HTTP-level suite checks
    hold at any hour of the day: the trigger fires, and the result is HELD rather than sent.
    """
    event = sample_cases.as_recent(sample_cases.CONSEQUENTIAL_EVENT)
    return {
...
def build_service(container: Container) -> OutreachService:
    """The pipeline, assembled from the REAL local adapters, exactly as every surface does.

    A test that hand-assembled its own service could quietly bind a different drafter or skip a
    port, and then prove nothing about what ships. One builder, used by the suite and mirrored
    by ``api/app.py``, ``cli/main.py`` and ``agent/tools.py``.
    """
...
@pytest.fixture()
def outreach_service(container: Container) -> OutreachService:
