...
* ``local``  : the offline family ANSWERS. It records what it was given, resolves a real
  principal, detects typed events, answers a consent question and counts a send, produces a
  routing reference. A silently empty offline stack is the failure mode that lets a repo ship a
  green gate and no working profile.
* ``onprem`` : the migration placeholder RAISES ``NotImplementedError``. A placeholder that
  returned successfully would be a false portability claim, and on three of these ports it
  would be worse than that: a delivery placeholder that returned a receipt would count a
  contact that never happened against a customer's cap, a consent placeholder that returned
  anything would be inventing a legal position, and a review router that returned would convert
  every consequential result into an unreviewed one.
...
from proactive_outreach.adapters._review_payload import (
    result_to_review,
)
...
def test_the_payload_that_reaches_the_wire_is_redacted_whichever_family_built_it() -> None:
    ...
    result = build_service(container).evaluate(
        sample_cases.PII_EVENT, actor=sample_cases.ACTOR, as_of=sample_cases.OPEN_INSTANT
    )
    payload = repr(
...
def test_the_whole_pipeline_answers_on_local_and_fails_fast_on_onprem() -> None:
    ...
    result = build_service(local).evaluate(
        sample_cases.CONSEQUENTIAL_EVENT,
        actor=sample_cases.ACTOR,
        as_of=sample_cases.OPEN_INSTANT,
    )
    assert result.requires_human_review is True
    assert result.delivered is False, "a consequential outreach was delivered without a human"
