...
from proactive_outreach.adapters.gcp.identity import (
    _AUDIENCE_ENV,
...
from proactive_outreach.ports.identity import (
    VERIFIED,
    EndUserAuthUnavailableError,
    declared_end_user_auth,
)

from tests.conftest import is_blocked_sdk, serving_body
...
_API_MODULE = "proactive_outreach.api.app"
...
def _rebound_settings() -> str:
    """The managed profile with every DATA port rebound to its SDK-free adapter.

    Generated from ``DEFAULT_BINDINGS`` rather than typed out, so a port added to the service
    is covered here automatically. Typing three of them by hand is how this file came to
    describe a service that no longer existed: the settings loader refuses a partial
    ``adapters:`` block, so the omission would have been a boot failure in a suite whose whole
    subject is what the process does at boot.

    IDENTITY keeps its managed binding, because it is the adapter under test.
    """
    from proactive_outreach.config import DEFAULT_BINDINGS

    lines = ['audit_path: ":memory:"', "iap_audience: " + "${" + _AUDIENCE_ENV + ":-}", "adapters:"]
    for port, table in sorted(DEFAULT_BINDINGS.items()):
        lines.append(f"  {port}:")
        for profile in ("local", "gcp", "onprem"):
            target = table[profile]
            if profile == "gcp" and port != "identity":
                target = table["local"]
...
def _managed_app(monkeypatch: pytest.MonkeyPatch, tmp_path: Any, audience: str) -> Any:
    ...
    different, louder case: the settings loader refuses it, naming the variable and its
    documented default, which ``tests/unit/test_settings_file.py`` pins.
    """
