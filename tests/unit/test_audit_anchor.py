...
_ANCHOR_ENV = "OUTREACH_AUDIT_ANCHOR"
...
def _record_three(settings: Settings) -> LocalAuditAdapter:
    ...
    for event in (
        ...
        service.evaluate(event, actor=sample_cases.ACTOR, as_of=sample_cases.OPEN_INSTANT)
    return audit
...
def test_an_append_after_truncation_cannot_relaunder_the_anchor(tmp_path: Path) -> None:
    ...
    # The SAME adapter instance, so the append goes through the log that holds the anchor.
    container = build_container(settings)
