...
#
# IAP is the deliberate difference from the reference edge. The reference keeps IAP off because
# its loader and iframe authenticate inside the application and an IAP redirect would break
...
# signature against IAP's own key set, its audience against OUTREACH_IAP_AUDIENCE, its expiry
...
#     deterministic body goes to a human, and an unconfigured channel means the notification is
#     reported undelivered rather than sent somewhere nobody chose.
#
...
locals {
  # The environment variables that are set ONLY when they carry a value. This service reads its
  # environment in THREE states, and a variable set to empty is an expressed intent that names
  # nothing, so writing "" here would be a different and louder posture than leaving the
  # variable off the service entirely:
  #   OUTREACH_IAP_AUDIENCE      emptied refuses every caller (503), by design
  #   OUTREACH_QUALITY_URL       emptied names no promotion authority, and the gate refuses
  #   OUTREACH_EVENT_VIEW        emptied means no source to detect from, and detection refuses
  #   OUTREACH_DRAFTING_MODEL    emptied means no drafter: deterministic body, human review
  ...
  optional_env = {
    OUTREACH_IAP_AUDIENCE       = var.iap_audience
    OUTREACH_QUALITY_URL        = var.quality_service_url
    OUTREACH_EVENT_VIEW         = var.event_view
    OUTREACH_DRAFTING_MODEL     = var.drafting_model
    OUTREACH_CHAT_AGENT         = var.chat_agent
    OUTREACH_SPEECH_VOICE       = var.speech_voice
    OUTREACH_TENANT             = var.service_tenant
    OTEL_EXPORTER_OTLP_ENDPOINT = var.otlp_endpoint
  }

  set_env = { for name, value in local.optional_env : name => value if value != "" }
}
...
resource "google_cloud_run_v2_service" "api" {
  ...
  template {
    ...
    encryption_key = one(google_kms_crypto_key.outreach[*].id)
    ...
    containers {
      ...
      # Set from the bucket this stack created, never from a variable: an operator who could
      # name the destination could name an out-of-region one, and synthesised audio is a
      # recording of what a named customer was told.
      env {
        name  = "OUTREACH_SPEECH_OUTPUT_URI"
        value = "gs://${google_storage_bucket.speech.name}"
      }

      dynamic "env" {
        for_each = local.set_env
        content {
          name  = env.key
          value = env.value
        }
...
resource "google_compute_security_policy" "api_per_source" {
  ...
  rule {
    ...
    description = "Per-source abuse boundary. A sweep costs a warehouse read, a consent question per event and a model call each, so the ceiling is at the edge rather than in the request path."
  }
...
# anyone; grant the service-operations group here, or at project scope if IAP membership is
