...
# OUTREACH_*, GCP_* and MKT_* names), so a deploy is "apply, then put these in the runtime
...
# ------------------------------ Data plane ---------------------------------- #
output "speech_output_uri" {
  description = "Regional CMEK bucket the synthesised voice notification is written to. App env: OUTREACH_SPEECH_OUTPUT_URI, which this stack sets itself when the edge is enabled."
  value       = "gs://${google_storage_bucket.speech.name}"
}

output "event_view" {
  description = "The client-owned warehouse view events are read from. App env: OUTREACH_EVENT_VIEW. This stack never creates it; when it lives in another project, grant app_service_account roles/bigquery.dataViewer there."
  value       = var.event_view
}

# ----------------------------- Service account ------------------------------ #
output "app_service_account" {
  description = "The serving identity. It holds no exportable key, and org policy forbids creating one. This is the principal to grant read on a cross-project event view."
...
output "iap_audience" {
  description = <<-EOT
    The exact value OUTREACH_IAP_AUDIENCE must carry: the IAP-protected backend service this
