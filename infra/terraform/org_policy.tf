...
# Require uniform bucket-level access on every bucket (no legacy object ACLs). The synthesised
# audio bucket holds a recording of what a named customer was told about their own account;
# per-object ACLs are how one becomes public.
