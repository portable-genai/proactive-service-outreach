...
#         into it, and it writes one for a REFUSAL as fully as for a send: in this vertical the
#         question asked after an incident is "why was this person not told", and the answer
#         has to be as immutable as the record of a contact that did happen. The offline
#         profile earns tamper evidence with a hash chain and an external head anchor; a
#         managed WORM sink needs no anchor because it provides non-rewritability itself,
#         which is why the anchor is documented as unnecessary here.
...
resource "google_logging_project_bucket_config" "worm_audit" {
  ...
  description    = "WORM audit bucket for the E5 Proactive Service Outreach service (six-month default retention)."
...
# without it a read of the outreach evidence leaves no trace. A consent-gated
# service that records who was contacted but not who read the record of it is
# half a trail.
# --------------------------------------------------------------------------- #
