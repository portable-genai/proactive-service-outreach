...
run "residency_defaults_are_in_country" {
  ...
  assert {
    condition     = google_kms_key_ring.outreach[0].location == var.region
...
run "serving_edge_contract" {
  ...
  assert {
    condition     = google_cloud_run_v2_service.api[0].template[0].encryption_key == google_kms_crypto_key.outreach[0].id
